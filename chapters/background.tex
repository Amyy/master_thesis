...
finite sets with $|X| = M$ (arbitrary length) and $|Y| = N$ (fixed length)~\cite{cha:bg_hashfunctoins_Stinson2006}. A hash function is considered \textbf{cryptographically secure} if it has the following  properties:~\cite{cha:bg_hashfunctions_BASIC_DEFINITIONS_Springer2004} 
...
% cryptographisch sichere hashfunktionen haben diese Eigenschaften?

\section{Digital Signature Schemes} % difference scheme -- system
A digital signature system is a digital device using a set of rules and a set of parameters enabling it to be used to verify the identity of the originator and integrity of data (i.e. messages)~\cite{cha:bg_digital_sign_schemes_NIST_standard1992}. These rules and parameters are 
