...
\section{Digital Signature Schemes} % difference scheme -- system
A \textit{digital signature scheme} uses a set of rules and a set of parameters to verify the identity of the originator and integrity of data.~\cite{cha:bg_digital_sign_schemes_NIST_standard1992} In this section, data refers to a message being sent from a sender (signer) to a recipient (verifier) across a network. % hier schutzziele erwähnen -> recipient will bestätigung das nachricht wirklich vom empfänder, sender will dass recipient ihm keine falschen nachrichten unterschiebt

% "signer/sender, verifier/recipient" -> so okay?
% ! digital signature scheme -> assymetrisch (immer?) 

The digital signature scheme consists of a key generation algorithm, a signature algorithm and a corresponding verification algorithm. % gehört keygen wirklich zu signatursystem?
The key generation algorithm creates a private key of the sender used for signature generation, and a public key used by the recipient for signature verification. % und auch noch von weiteren Leuten die Originalität von Nachricht prüfen wollen? Aber in meinem runtergebrochenen Beispiel nur 2 Leute
The signature algorithm generates the \textit{digital signature} of a message by using the secret known only to the signer and the content of the message being signed.
The verification algorithm is used by the recipient to check the authenticity of the signature and the corresponding message with the public key.
~\cite{cha:bg_signature_schemes_book_menezes2018_1997}
% assymetric system - public, private key
% schema "keygen, signieren, verify"
...
finite sets with $|X| = M$ (arbitrary length) and $|Y| = N$ (fixed length)~\cite{cha:bg_hashfunctoins_Stinson2006}. A hash function is considered \textit{cryptographically secure} if it has the following  properties:~\cite{cha:bg_hashfunctions_BASIC_DEFINITIONS_Springer2004} 
